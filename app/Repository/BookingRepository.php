...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        // Add the 'usercheck' attribute to each job using the map technique.
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        if ($cuser && $cuser->is('customer')) {
            ...
            $usertype = 'customer';
            return ['emergencyJobs' => $emergencyJobs, 'normalJobs' => [], 'jobs' => $jobs, 'cuser' => $cuser, 'usertype' => $usertype];
        } elseif ($cuser && $cuser->is('translator')) {
            $jobs = Job::getTranslatorJobsHistoric($cuser->id, 'historic', $perPage, $page);
    ...
    public function store($user, $data)
    {
        if ($user->user_type !== env('CUSTOMER_ROLE_ID')) {
            return [
                'status' => 'fail',
                'message' => "Translator can not create a booking"
            ];
        }
        ...
        $data['certified'] = $certifiedMap[$data['certified']] ?? null;

        // Handle special cases for job_for
    ...
    public function jobEnd($post_data = [])
    {
        ...
        $diff = date_diff($completedDate, $dueDate);
        ...
        $this->sendSessionEmail($job, $job->user, 'faktura', $sessionTime);
        
        $translator = $job->translatorJobRel->where('completed_at', null)->where('cancel_at', null)->first();
        if ($translator) {
            $this->sendSessionEmail($job, $translator->user, 'lön', $sessionTime);
    ...
    private function sendSessionEmail($job, $user, $forText, $sessionTime)
    {
        $email = $user->email;
        $name = $user->name;
    ...
    public function getPotentialJobIdsWithUserId($user_id)
    {
        ...
        $jobs = TeHelper::convertJobIdsInObjs($filtered_job_ids);
        return $jobs;
    }
    ...
    public function sendNotificationTranslator($job, $data = [], $exclude_user_id)
    {
        ...
        $language = TeHelper::fetchLanguageFromJobId($from_language_id);
        ...
        $users = User::where('user_type', '2')
                     ...
                     ->whereNotIn('id', function ($query) {
                         $query->select('user_id')
                               ->from('user_metas')
                               ->where('meta_key', 'not_get_emergency')
        ...
        foreach ($users as $oneUser) {
            ...
            if ($immediate == 'yes' && TeHelper::getUsermeta($userId, 'not_get_emergency') == 'yes') {
                continue;
            }
    
            $jobs = $this->getPotentialJobIdsWithUserId($userId);
        ...
        $notification_type = 'suitable_job';
        $msg_contents = $immediate == 'no' ? "Ny bokning för {$language} tolk {$data['duration']}min {$data['due']}" :
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        $jobPosterCity = $job->city ? $job->city : optional($job->userMeta)->city;
        ...
        $physicalJobMessageTemplate = trans('sms.physical_job', compact('date', 'time', 'jobPosterCity', 'duration', 'jobId'));
    ...
    /**
     * Function to check if need to delay the push
     * @param $user_id
     * @return bool
     */
    ...
    public function sendPushNotificationToSpecificUsers($users, $job_id, $data, $msg_text, $is_need_delay)
    {
        ...
        $logger = new Logger('push_logger');
        $logger->pushHandler(new StreamHandler(storage_path("logs/push/laravel-{$env}.log"), Logger::DEBUG));
        $logger->pushHandler(new FirePHPHandler());
